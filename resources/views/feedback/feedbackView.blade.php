            ...
            <div class="">
                @if($data->id_user == Auth::id())
								...
								@endif
								 ...
								 @if($admin ->id == Auth::id() && $data->statusFeedback !==2)
								 <a href="{{ route('feedbackPublishSubmit', $idFeedback = $data->id) }}" class="btn btn-default active" style="background-color:#ffc1072b; width: 200px; margin-right: 10px;">Опубликовать</a>
								 <a href="{{ route('feedbackDelete', $idFeedback = $data->id) }}" class="btn btn-default active" style="background-color:#dbb5a199; width:200px; ">Удалить отзыв</a>
								@endif
          	</div>
						<br>
